...
function_wilcox <- function(data, analysis_thing, out){
    ...
    genus_diff_stat <- do.call(rbind, lapply(genus, function(x){
        ...
        if(sum(data$analysis_genus) > 0){  # filter all 0 genus, because subet
    ...
    #write.table(genus_diff_stat_sig, str_glue("{outdir}/{analysis_thing}_wilcox_test_sig.tsv"), sep = "\t", quote = F, row.names = F)
}
