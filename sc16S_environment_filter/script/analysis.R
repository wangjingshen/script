...
# args --
argv <- arg_parser('')
#argv <- add_argument(argv, "--genus_df", help = "genus_df")
argv <- add_argument(argv, "--fj_path", help = "fj path")
argv <- add_argument(argv, "--sample", help = "sample")
argv <- parse_args(argv)

#genus_df_list <- unlist(strsplit(argv$genus_df, split = ","))
...
function_filter <- function(fj_path, sample){
    ...
    if(!dir.exists(sample)){
        dir.create(sample, recursive = TRUE)
    }

    genus_df <- read.table(str_glue('{fj_path}/{sample}/04.analysis/{sample}_genus_umi.tsv'), sep = "\t", row.names = 1, header = T)
    ...
    write.table(cbind.data.frame(barcode = row.names(genus_df_filter),
                                 genus_df_filter), 
               str_glue('{sample}/{sample}_genus_umi_filter.tsv'), sep = "\t", quote = F, row.names = F)
    ...
    write.table(genus_bc, str_glue('{sample}/{sample}_genus_detect_barcode.tsv'), sep = "\t", quote = F, row.names = F)
    ...
    write.table(genus_umi, str_glue('{sample}/{sample}_genus_detect_umi.tsv'), sep = "\t", quote = F, row.names = F)
}
